...
// defaults write IDEPackageSupportDisablePluginExecutionSandbox -bool YES
...
@main
struct SwiftonizeBuilderNew: BuildToolPlugin {
	...
	func createBuildCommands(context: PluginContext, target: Target) async throws -> [Command] {
		...
		//        }
		//throw SwiftonizeError.fileNotFound(path: target.directory.string)
		var arguments: [CustomStringConvertible] = [
			"extension2",
			input,
			context.pluginWorkDirectory,
			//"--site",
			//"/Users/codebuilder/Library/Mobile Documents/com~apple~CloudDocs/Projects/xcode_projects/touchBay_files/rebuild/touchBay/venv_test"
			//			python_stdlib,
			//			python_extra
			
		]
		let generatorTool = try context.tool(named: "SwiftonizeExecutable")
		return [
			.buildCommand(
...
extension SwiftonizeBuilderNew: XcodeBuildToolPlugin {
...
extension SwiftonizeBuilderNew {
